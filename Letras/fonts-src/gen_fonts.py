...
Barriecito TTF. Glifos en blanco con cobertura en canal alfa (CIQ los tinta
con setColor). Empaquetado tipo 'shelf' en un atlas RGBA.

Esta esfera escribe la hora en palabras, asi que solo necesita 17 letras: las
que aparecen en DOCE UNA DOS TRES CUATRO CINCO SEIS SIETE OCHO NUEVE DIEZ ONCE
EN PUNTO Y CUARTO VEINTE VEINTI MEDIA MENOS. El atlas queda diminuto.
"""
import os
...
# Solo las letras que de verdad se usan (ver docstring). Comprobado a mano.
WORD_CHARS = "ACDEHIMNOPRSTUVYZ"
...
#   Link  -> la linea del enlace (Y / MENOS / EN), siempre pequena
#   WordA/LinkA -> la version del Always-On, recortada para caber bajo el
#            10% de pixeles encendidos (medido: 6,6% en el caso peor)
JOBS = [
    ("Word",  "Barriecito-Regular.ttf", 116, WORD_CHARS, False),
    ("WordS", "Barriecito-Regular.ttf", 100, WORD_CHARS, False),
    ("Link",  "Barriecito-Regular.ttf", 64,  WORD_CHARS, False),
    ("WordA", "Barriecito-Regular.ttf", 64,  WORD_CHARS, False),
    ("LinkA", "Barriecito-Regular.ttf", 38,  WORD_CHARS, False),
]
