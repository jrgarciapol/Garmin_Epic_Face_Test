...
//! Esfera digital para el Epix Pro 51 mm (454 x 454, AMOLED) — variante
//! PULSO XL: idéntica a `Pulso`, salvo que aquí el TAMAÑO de los orbes también
//! es proporcional a las pulsaciones (en `Pulso` es fijo, 11 px de radio).
//! Base Barriecito con sus tamaños de texto intactos.
//!
...
class EpixWatchFaceView extends Ui.WatchFace {
    ...
    // El radio del orbe NO es constante en esta variante: lo calcula
    // dotRadius() a partir de las pulsaciones.
    ...
    //! Radio del orbe, proporcional al pulso: es la diferencia de esta variante
    //! frente a `Pulso`, donde era fijo (11 px). A 60 ppm es un punto discreto
    //! y a 160 ppm una bola gorda que se ve de reojo sin fijar la vista.
    //! Referencias:  50 -> 7 · 60 -> 9 · 75 -> 12 · 100 -> 17 · 130 -> 23 ·
    //!               160 -> 29 px.  El halo llega a 1,9 veces eso.
    //! La estela y el arranque del choque se escalan solos, porque se miden en
    //! múltiplos de este radio.
    private function dotRadius(hr) {
        var r = 9.0 + (hr - 60.0) * 0.20;
        if (r < 6.0)  { r = 6.0; }
        if (r > 34.0) { r = 34.0; }
        return r;
    }
    ...
    //! Cabeza del orbe: halo, cuerpo y núcleo. `rDot` lo marca el pulso.
    private function drawHead(dc, cx, cy, deg, t, warm, rDot) {
        var x = orbX(cx, deg);
        var y = orbY(cy, deg);
        dc.setColor(orbColor(t, warm, 0.25), Gfx.COLOR_TRANSPARENT);
        dc.fillCircle(x, y, (rDot * 1.9).toNumber());
        dc.setColor(orbColor(t, warm, 0.60), Gfx.COLOR_TRANSPARENT);
        dc.fillCircle(x, y, (rDot * 1.4).toNumber());
        dc.setColor(orbColor(t, warm, 1.0), Gfx.COLOR_TRANSPARENT);
        dc.fillCircle(x, y, rDot.toNumber());
    }
